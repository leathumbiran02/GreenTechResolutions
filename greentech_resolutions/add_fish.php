<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        $add_community_fish=$connect->prepare("
        INSERT INTO
        community_fish
        (user_email,fish_name,description,image,price)
        VALUES
        (?,?,?,?,?)
        ");

        $add_community_fish->bind_param("sssss",$user_email,$fish_name,$description,$image,$price); /* Bind the parameters: */
        $add_community_fish->execute(); /* Execute the statement: */

        /* Check if the details were updated successfully in the vendor table: */
        if($add_community_fish->affected_rows >0){
            echo '<script>alert("Your fish was successfully added!")</script>';
